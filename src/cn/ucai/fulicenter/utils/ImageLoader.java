...
import cn.ucai.fulicenter.BitmapUtils;
import cn.ucai.fulicenter.HttpUtils;
...
public class ImageLoader {
	...
	class DownloadImageTask extends AsyncTask<ImageBean, Void, ImageBean>{
		...
		protected ImageBean doInBackground(ImageBean... params) {
			...
			try {
				...
				//缓存至sd卡
				BitmapUtils.saveBitmap(bean.bitmap, cn.ucai.fulicenter.utils.FileUtils.getDir(mContext,bean.imgName));
				bean.msg="图片下载成功";
			} catch (Exception e) {
				bean.msg="图片下载失败";
			}finally{
	...
	public Bitmap displayImage(String path,String imgName,int width,int height,OnImageLoadListener listener){
		...
		//若图片缓存在sd卡，则直接返回该图片
		Bitmap bitmap = BitmapUtils.getBitmap(cn.ucai.fulicenter.utils.FileUtils.getDir(mContext,imgName));
		if(bitmap!=null){
			return bitmap;
		}
